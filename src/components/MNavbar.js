    ...
    <div className="w-full px-3 md:px-7 py-1 md:py-2">
        <div className={"container mx-auto flex flex-col md:flex-row justify-between items-center text-"+textcolor}>
            ...
            <nav className="flex mb-4 md:mb-0">
                ...
                {(user)?<Image src={user.photoURL} className="rounded-full p-1" width={50} height={50}/>:<Link href={"/login"}><button type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Login</button></Link>}
            </nav>
        </div>
    </div>
    );
